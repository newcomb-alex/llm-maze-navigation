...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run N independent maze simulations.")
    p.add_argument("--map", dest="map_path", required=True, help="Path to map txt, e.g. maps/easy_01.txt")
    p.add_argument("--runs", type=int, default=10, help="Number of independent runs in this batch")
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Base random seed (optional). Run i uses seed+i so the batch is reproducible.",
    )
    p.add_argument(
        "--stop-on-error",
        action="store_true",
        help="Abort the batch if a run raises. Default is to log the error and continue.",
    )
    return p.parse_args()
...
def run_one(run_id: str, args: argparse.Namespace, map_lines: list[str], used_seed: int) -> None:
    """
    Execute a single, fully independent run: fresh seed, fresh controller
    instances, fresh simulator, and its own log + summary files on disk.
    """
    random.seed(used_seed)

    # Rebuilt per run so no controller state leaks between runs.
...
def main() -> None:
    ...
    for i in range(1, args.runs + 1):
        ...
        run_id = f"{batch_id}_run{i:0{width}d}"

        print(f"--- Run {i}/{args.runs} ---")
        try:
            run_one(run_id, args, map_lines, used_seed)
            completed += 1
        except KeyboardInterrupt:
            print(f"\nInterrupted during run {i}/{args.runs}. Stopping batch.")
            break
        except Exception:
            failed += 1
            print(f"Run {run_id} FAILED (seed={used_seed}):")
            traceback.print_exc()
    ...
    print(f"Batch ID: {batch_id}")
    print(f"Completed: {completed}/{args.runs}   Failed: {failed}")
